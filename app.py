...
cgce_selecionados = st.sidebar.multiselect("Tipo de Contrato", cgce_disponiveis, default=cgce_disponiveis)
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_ncm = px.bar(
            ...
            labels={'Valor US$ FOB': 'Valor comércio anual (USD)', 'Descrição NCM': ''}
        )
...
with col_graf2:
    if not df_filtrado.empty:
        top_cgce = df_filtrado.groupby('Descrição CGCE Nível 1')['Valor US$ FOB'].sum().nlargest(10).sort_values(ascending=True).reset_index()
        grafico_cgce = px.bar(
            top_cgce,
            x='Valor US$ FOB',
            y='Descrição CGCE Nível 1',
            orientation='h',
            title="Top 10 CGCE por valor total",
            labels={'Valor US$ FOB': 'Valor comércio anual (USD)', 'Descrição CGCE Nível 1': ''}
        )
...
with col_graf3:
    if not df_filtrado.empty:
        ...
        grafico_cuci = px.bar(
            ...
            labels={'Valor US$ FOB': 'Valor comércio anual (USD)', 'Descrição CUCI Seção': ''}
        )
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        grafico_isic = px.bar(
            ...
            labels={'Valor US$ FOB': 'Valor comércio anual (USD)', 'Descrição ISIC Seção': ''}
        )
